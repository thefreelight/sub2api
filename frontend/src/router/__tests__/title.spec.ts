...
describe('resolveDocumentTitle', () => {
  ...
  it('站点名为空时，回退默认站点名', () => {
    expect(resolveDocumentTitle('Dashboard', '')).toBe('Dashboard - NavtoAI API')
    expect(resolveDocumentTitle(undefined, '   ')).toBe('NavtoAI API')
  })

  it('官方默认站点名归一为 NavtoAI 品牌', () => {
    expect(resolveDocumentTitle('Home', 'Sub2API')).toBe('Home - NavtoAI API')
  })
