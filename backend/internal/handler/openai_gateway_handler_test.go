...
func TestOpenAISameAccountRetryLimit_ProtectedFallback(t *testing.T) {
	protected := &service.Account{Credentials: map[string]any{"never_temp_unschedulable": true}}
	normal := &service.Account{}

	require.Equal(t, protectedFallbackRetryLimit, openAISameAccountRetryLimit(protected, &service.UpstreamFailoverError{StatusCode: http.StatusBadGateway}))
	require.Equal(t, protectedFallbackRetryLimit, openAISameAccountRetryLimit(protected, &service.UpstreamFailoverError{StatusCode: http.StatusTooManyRequests}))
	require.Zero(t, openAISameAccountRetryLimit(protected, &service.UpstreamFailoverError{StatusCode: http.StatusForbidden}))
	require.Zero(t, openAISameAccountRetryLimit(normal, &service.UpstreamFailoverError{StatusCode: http.StatusBadGateway}))
}
