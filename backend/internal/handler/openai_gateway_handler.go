...
const protectedFallbackRetryLimit = 2
...
func openAISameAccountRetryLimit(account *service.Account, failoverErr *service.UpstreamFailoverError) int {
	...
	if status == http.StatusRequestTimeout || status == http.StatusTooManyRequests || status >= http.StatusInternalServerError {
		return protectedFallbackRetryLimit
	}
	return 0
}
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					// 池模式和强制兜底账号：瞬时故障优先在同账号重试。
					if retryLimit := openAISameAccountRetryLimit(account, failoverErr); retryLimit > 0 {
						allowed := sameAccountRetryCount[account.ID] < retryLimit
						if failoverErr.RetryableOnSameAccount {
