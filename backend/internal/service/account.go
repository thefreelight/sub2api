...
// IsTempUnschedulableProtected keeps designated last-resort accounts in the
// scheduler when an upstream error would otherwise apply a temporary block.
func (a *Account) IsTempUnschedulableProtected() bool {
	if a == nil || a.Credentials == nil {
		return false
	}
	protected, ok := a.Credentials["never_temp_unschedulable"].(bool)
	return ok && protected
}
